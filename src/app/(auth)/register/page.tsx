...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterInput) => {
    try {
      ...
      router.push("/verify-otp?type=email_verification");
    } catch (err: any) {
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <div className="text-center mb-8">
          <Link href="/" className="inline-flex items-center gap-2">
            <div className="h-10 w-10 rounded-2xl bg-gradient-to-br from-orange-500 to-pink-500 flex items-center justify-center">
          ...
          <p className="text-gray-500">Join millions of food lovers</p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-xl border border-gray-100 dark:border-gray-800 p-8">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <Input label="Full name" placeholder="John Doe" leftIcon={<User className="h-4 w-4" />} error={errors.name?.message} {...register("name")} />
            ...
            <Input label="Phone number" type="tel" placeholder="+234 800 000 0000" leftIcon={<Phone className="h-4 w-4" />} error={errors.phone?.message} {...register("phone")} />
            <Input
              label="Password"
            ...
            <Input label="Referral code (optional)" placeholder="Enter referral code" error={errors.referralCode?.message} {...register("referralCode")} />
          ...
          <button onClick={() => window.location.href = "/api/auth/google"}
            className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl border-2 border-gray-200 dark:border-gray-700 hover:border-gray-300 transition-colors text-sm font-medium text-gray-700 dark:text-gray-300">
